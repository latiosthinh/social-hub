...
export const getAccounts = (userId: string) => {
    return getDb().prepare('SELECT * FROM social_accounts WHERE user_id = ?').all(userId);
};
...
export const toggleAccount = (id: string, isActive: boolean) => {
    getDb().prepare('UPDATE social_accounts SET is_active = ? WHERE id = ?').run(isActive ? 1 : 0, id);
};

export const toggleGroup = (userId: string, platform: string, isActive: boolean) => {
    getDb().prepare('UPDATE social_accounts SET is_active = ? WHERE user_id = ? AND platform = ?').run(isActive ? 1 : 0, userId, platform);
};
